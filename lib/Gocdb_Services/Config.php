...
class Config {
    ...
    private function readLocalInfoXML ($path, $url = NULL) {
        ...
        // Validate the the configuration file xml
        
        $dom = dom_import_simplexml($base);

        // Cludge: we (have to) assume the schema is in the same directory as the config file.
        // If there is no schema file then we SILENTLY carry on without validation.
        // If you don't want to validate the schema every time the file is loaded, simply move
        // the .xsd file out of the way. Everything except for offline validation should still
        // work. (but you won't throw errors from an invalid file here!)
        $schemaPath = preg_replace('/xml$/', 'xsd', $path);

        if (file_exists($schemaPath) and !$dom->ownerDocument->schemaValidate($schemaPath)) {
            $this->throwXmlErrors('Failed to validate configuration file against schema '.$path);
        }            
        ...
        $default_info->registerXPathNamespace($this->xmlnsPrefix, $this->xmlnsURI);
        
        if (!is_null($url)) {
            // Find any elements matching the given url
            $qualified = $base->xpath("//$this->xmlnsPrefix:local_info[@url=\"$url\"]");
    ...
    private function descendXml (\SimpleXmlIterator $iterator, $keys, \SimpleXmlElement $default_info) {      
        ...
        for ($iterator->rewind(); $iterator->valid(); $iterator->next()) {
            ...
            // Seems not to be necessary but .... belt and braces with namespaces.
            $iterator->registerXPathNamespace($this->xmlnsPrefix, $this->xmlnsURI);
            ...
            if ($iterator->hasChildren()) {
                ...
                $p = "$this->xmlnsPrefix:".implode("/$this->xmlnsPrefix:",$keys);
                ...
                if (count($elem)) {
                    ...
                    throw new \ErrorException("Input configuration file override element ".implode('/',$keys)." not found in default section: ".$iterator->key().' => '.$iterator->current());
                }
